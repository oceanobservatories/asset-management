...
%.. desiderio 2023-05-16: added capability of processing all pairs of
%..                       files when all are in the same folder
%
%                      
%        To test this function, put one SUNA calfile and the OOI csv calfile
%        generated from it in a folder containing no other files. Change the 
%        Matlab working directory to this folder and then run this function 
%        with no arguments.
%
%
...
%
%.. Change working directory to one containing a number of NUTNR calfiles
%.. and the corresponding csv calfiles created from them:
%..
%.. DO NOT USE STRING DATATYPE FOR XLSX FILENAMES. 
%..
%.. The xlsx comparison file(s) will be created in the same directory.
%
%##########################################################################
...
listing = sort(strtrim(cellstr(ls(pwd))));
...
listing = strrep(listing, '\\', '\');    % if root wdir then double slash results 
%.. need to take into account '.cal' vs. 'CAL' designations
calFilenames = listing(contains(listing, '.cal', 'IgnoreCase', true));  % vendor calfile
csvFilenames = listing(contains(listing, '.csv'));                      % OOI calfile
xlsxFilenames = strrep(calFilenames, '.cal', '.xlsx');                  % testbed file
xlsxFilenames = strrep(xlsxFilenames, '.CAL', '.xlsx');                 % testbed file

%.. delete testbed file(s) if it/they exist
xlsxFilenames(isfile(xlsxFilenames)) = [];
warning('off', 'MATLAB:xlswrite:AddSheet');

%.. loop through all serial numbers. It is assumed that there is a one-to-one
%.. matching of csv to cal file index by serial number.
for jj = 1:numel(calFilenames)
    %.. this code originally processed only 1 calfile. Therefore keep
    %.. those variable names by using these assignment statements:
    calFile  = calFilenames{jj};
    csvFile  = csvFilenames{jj};
    xlsxFile = xlsxFilenames{jj};
    %.. and clear cell arrays to work with multiple passes
    clearvars C CC D csvSheet 
    %.. trust ... but verify
    %.. track serial numbers in filenames from the end.
    calSerialNumber = str2double(calFile(end-8:end-5));
    csvSerialNumber = str2double(csvFile(end-18:end-14));
    if calSerialNumber ~= csvSerialNumber
        error('calfile and csvfile serial numbers don''t match up.');
    end
